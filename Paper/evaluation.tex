...
Finally, we compare \method~ against defence schemes (Section~\ref{eval-defences}). %state of the art
...
for space reason\footnote{the two other datasets (capturing preferences of online customers and capturing location check-ins~\cite{shokri2017membership}) depict similar results}.%in the main text and the other two datasets in the Appendix
...
%at the end in Figure~\ref{fig:retrain}.
...
\textbf{Mitigating the Privacy Risks in Pruned Models.} We describe a potential approach to mitigate the privacy risk of the compressed models without requiring to modify the model's training.
